...
1. You application implementation (preferably not web-facing, for security reasons). This contains a reference to the `ccApp` class definition, i.e., the ccPhp framework.
1. The `ccPhp` framework.
...
1. Set operating mode (development vs. production).
1. Specify the implementation class(es) which implement `ccPageInterface`.
1. Get an object representing the current HTTP request. (this may not be required in the future)
1. Call `ccApp::dispatch()`
